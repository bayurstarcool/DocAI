<script>
  ...
  let pageSize = 12
  let detailPage = 1
  let detailPageSize = 60
  let detailTotal = 0
  let filteredItems = []
  let selectedFolder = "input"
  ...
  $: {
    ...
    if (newSlug !== selectedSlug) {
      ...
      if (selectedSlug) {
        if (datasets.length === 0) loadDatasets().then(() => loadDetail())
        else loadDetail()
      } else {
        singleDataset = null
        singleImages = []
  ...
  async function loadDatasets() {
    ...
    try {
      ...
      datasets = (d.datasets || []).sort((a, b) => (b.updated_at || 0) - (a.updated_at || 0))
    } catch (e) {
  ...
  async function loadDetail(newPage) {
    if (newPage) detailPage = newPage
  ...
  $: {
    ...
    if (cat !== "all") items = items.filter((d) => d.kind === cat)
  ...
  function categoryCount(id) {
    if (id === selectedCategory) return filteredItems.length
    return datasets.filter((d) => d.kind === id).length
  }

  $: detailPages = Math.max(1, Math.ceil(detailTotal / detailPageSize))
  ...
  {#if !selectedSlug}
    ...
    <div class="filters">
      <div class="category-tabs">
        {#each [{ id: "all", label: "All" }, { id: "paired", label: "Paired" }, { id: "clean", label: "Clean" }, { id: "identity", label: "Identity" }] as cat}
    ...
    {:else}
      ...
      <p class="list-count">{filteredItems.length} dataset · urut terbaru dulu</p>
    {/if}
    ...
    {:else}
      <div class="detail-header">
        <div>
          ...
          <p class="detail-path">{singleDataset.slug}</p>
        </div>
        <div class="detail-stats">
          <span class="stat-badge">Paired</span>
          <span class="stat-badge">{singleDataset.paired_count || 0} pairs</span>
          <span class="stat-badge">{singleDataset.input_count || 0} input</span>
          <span class="stat-badge">{singleDataset.target_count || 0} target</span>
          <span class="stat-badge" class:ready={singleDataset.paired_count > 0}>
            {singleDataset.paired_count > 0 ? "Ready" : "Not Ready"}
          </span>
        </div>
      </div>
      ...
      <div class="folder-tabs">
        <button class="folder-tab" class:active={selectedFolder === "input"} onclick={() => switchFolder("input")}>
          <i data-lucide="folder"></i> Input ({singleDataset.input_count || 0})
        </button>
        <button class="folder-tab" class:active={selectedFolder === "target"} onclick={() => switchFolder("target")}>
          <i data-lucide="folder"></i> Target ({singleDataset.target_count || 0})
        </button>
        <button class="folder-tab" class:active={selectedFolder === "all"} onclick={() => switchFolder("all")}>
          <i data-lucide="layers"></i> All
        </button>
      </div>

      {#if singleImages.length === 0}
        <p class="empty-text">Tidak ada gambar ditemukan di folder ini.</p>
      {:else}
        ...
        <div class="pagination">
          <button disabled={detailPage <= 1} onclick={() => loadDetail(detailPage - 1)}>
            Sebelumnya
          </button>
          <span>
            Halaman {detailPage} / {detailPages} · {detailTotal} gambar
          </span>
          <button disabled={detailPage >= detailPages} onclick={() => loadDetail(detailPage + 1)}>
            Berikutnya
          </button>
        </div>
      {/if}
...
<style>
  ...
  .list-count { color: var(--text3); font-size: 0.8rem; margin-top: 1rem; text-align: center; }
  .pagination { display: flex; justify-content: center; align-items: center; gap: 0.75rem; margin-top: 1.25rem; color: var(--text2); font-size: 0.8rem; }
  .pagination button { padding: 0.4rem 0.75rem; border: 1px solid var(--border); background: var(--bg2); color: var(--text); border-radius: 6px; cursor: pointer; }
  ...
  /* Folder tabs */
  .folder-tabs { display: flex; gap: 0.25rem; margin-bottom: 1rem; }
  .folder-tab { display: inline-flex; align-items: center; gap: 0.35rem; padding: 0.45rem 1rem; border: 1px solid var(--border); border-radius: 6px; background: transparent; color: var(--text2); font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.15s; }
  .folder-tab:hover { border-color: var(--accent); color: var(--text); }
  .folder-tab.active { background: rgba(6, 182, 212, 0.1); color: var(--accent2); border-color: rgba(6, 182, 212, 0.3); }
